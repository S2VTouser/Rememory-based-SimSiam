...
class Der(ContinualModel):
    ...
    def observe(self, inputs1, labels, inputs2, notaug_inputs):
        ...
        self.opt.zero_grad()
        #print('self.args.cl_default:',self.args.cl_default)
        if self.args.cl_default: #SCL
            ...
            outputs = self.net.module.backbone(inputs1.to(self.device)) #resnet last laye(fc) out dim=classes
            data_dict['penalty'] = 0
        ...
        loss.backward()
        self.opt.step()
        #print('buffer size:', self.buffer.num_seen_examples)
