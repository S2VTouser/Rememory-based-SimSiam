...
def store_masked_loaders(train_dataset: datasets, test_dataset: datasets, memory_dataset: datasets, 
    ...
    train_mask = np.logical_and(np.array(train_dataset.targets) >= setting.i,
        np.array(train_dataset.targets) < setting.i + setting.N_CLASSES_PER_TASK) #返回True/False i=0,则选了类0，1
    test_mask = np.logical_and(np.array(test_dataset.targets) >= setting.i,
        np.array(test_dataset.targets) < setting.i + setting.N_CLASSES_PER_TASK)

    train_dataset.data = train_dataset.data[train_mask] #mask掉false
    test_dataset.data = test_dataset.data[test_mask]

    # print('mask~~:',test_dataset.targets[:30]) #从0到9标签

    train_dataset.targets = np.array(train_dataset.targets)[train_mask] #i=0时，只有0，1
    test_dataset.targets = np.array(test_dataset.targets)[test_mask]

    # print('train data after mask:', len(train_dataset.data)) #10000
    print('test data target after mask:', test_dataset.targets[:10])   #2000
    ...
    train_loader = DataLoader(train_dataset,
                              batch_size=setting.args.train.batch_size, shuffle=True, num_workers=1)
    test_loader = DataLoader(test_dataset,
                             batch_size=setting.args.train.batch_size, shuffle=False, num_workers=1)
