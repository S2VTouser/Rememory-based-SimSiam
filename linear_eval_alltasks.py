...
def main(device, args):
    ...
    for t in tqdm(range(0, dataset_copy.N_TASKS), desc='Evaluating'):
      ...
      msg = model.net.module.backbone.load_state_dict({k[16:]:v for k, v in save_dict['state_dict'].items() if 'backbone.' in k and 'k_backbone.' not in k}, strict=True)
    ...
    print(f'KNN accuracy on Task {t1}: {mean_knn_acc}') #最后一个任务在各个任务上的acc
